...
import { Acciones, columnsFilter, columnsHeadersTtipocomp, columnsOrder } from "./header-table";
...
export const Ttipocomp = () => {

  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const [rows, setRows] = useState<ITtipocomp[]>([]);
  const [order, setOrder] = useState<Order[]>([
    { column: "Tipo", id: "tipocompra", operator: "DESC" },
  ]);
  ...
  useEffect(() => {
    setRows(data?.tipo_doc_com_list || []);
    setCount(data?.total);
  }, [data]);
  ...
  const handleConfirmDelete = () => {
    const rowToDelete = rows.find((row) => row.tipocompra === deleteRowId);
    if (rowToDelete) {
    } else {
    ...
    <>
      ...
      >
        <BaseTable
          loading={isLoading}
          rows={rows}
          headers={columnsHeadersTtipocomp }
