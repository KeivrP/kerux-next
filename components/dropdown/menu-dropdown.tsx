  ...
  return (
    <div>
      <button
        ...
        className="flex items-center text-sm pe-1 font-medium text-gray-900 rounded-full hover:text-[#142F62]  dark:text-white"
        type="button"
        onClick={handleClick}
      >
        <span className="sr-only">Abrir menú de usuario</span>
        {icon && <span className="mr-2">{icon}</span>}
        <span className="capitalize">{buttonText.toLowerCase()}</span>
        <svg
